...
export default function GoalContributionModal({ goal, onClose, onSave, currency, selectedMonth, available, availableForDate }) {
  ...
  const maximumAssignment = Math.max(Math.min(datedAvailable, goal?.remaining ?? datedAvailable), 0);
  ...
  return (
    <ModalShell open={Boolean(goal)} onClose={onClose} eyebrow="Assign funds" title={goal ? `Fund ${goal.name}` : "Fund goal"}>
      <form onSubmit={submit} className="px-5 pb-6 pt-6 sm:px-7 sm:pb-7">
        <div className="rounded-xl bg-forest-50 p-3 text-xs text-forest-800"><strong>{formatMoney(Math.max(datedAvailable, 0), currency)}</strong> available in the assignment month before this assignment.</div>
