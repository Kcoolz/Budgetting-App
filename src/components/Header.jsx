import { ChevronLeft, ChevronRight, Download, Moon, Plus, Sun } from "lucide-react";
...
export default function Header({
  ...
  return (
    <header className={`app-header sticky top-0 z-10 flex min-h-[72px] items-center justify-between border-b border-black/5 px-4 backdrop-blur-xl sm:px-6 lg:justify-end lg:px-10 xl:px-14 ${profileType === "business" ? "bg-[#f3f7f5]/90" : "bg-cream-100/90"}`}>
      ...
      <div className="flex min-w-0 items-center gap-1.5 sm:gap-2.5">
        ...
        <button
          ...
          className="interactive-button grid size-9 shrink-0 place-items-center rounded-xl border border-black/8 bg-white text-slate-500 hover:border-forest-700/25 hover:bg-forest-50 hover:text-forest-800 sm:size-10"
        ...
        <div className="flex items-center rounded-xl border border-black/8 bg-white p-1">
          ...
          <label>
            ...
            <input type="month" value={selectedMonth} onChange={(event) => onMonthChange(event.target.value)} className="min-h-8 w-[82px] border-0 bg-transparent px-0.5 text-[10px] font-semibold text-ink-900 outline-none sm:w-[142px] sm:px-2 sm:text-xs" />
          </label>
